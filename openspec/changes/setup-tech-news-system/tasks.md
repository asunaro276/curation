...
- [x] Rubyプロジェクトの初期化（`bundle init`）
- [x] ディレクトリ構造の作成（`lib/`, `config/`, `spec/`）
- [x] Gemfileの作成と基本的な依存関係の追加
  - `anthropic` (Claude API)
  - `faraday` (HTTPクライアント)
  - `rss` (RSS解析)
  - `nokogiri` (HTML解析)
  - `rspec` (テストフレームワーク)
- [x] `.gitignore` の作成（`.env`, `*.gem`, `vendor/` 等）
- [x] `README.md` の作成（プロジェクト概要、セットアップ手順）
...
### Task 1.2: 設定管理の実装
- [x] `config/sources.yml` の作成（ニュースソース定義）
- [x] `lib/tech_news/config.rb` の実装
  - 環境変数読み込み（`ANTHROPIC_API_KEY`, `SLACK_WEBHOOK_URL`）
  - `sources.yml` のパース
  - 設定検証ロジック
- [x] `.env.example` の作成（必須環境変数の例）
- [x] 設定クラスのユニットテスト作成
...
- [x] `lib/tech_news/models/article.rb` の実装
...
- [x] `lib/tech_news/collectors/base.rb` の実装
  - 抽象メソッド定義（`fetch`, `parse`）
  - エラーハンドリング
  - タイムアウト設定
- [x] HTTPヘルパーメソッドの実装（User-Agent設定等）
- [x] ベースクラスのテスト作成
...
- [x] `lib/tech_news/collectors/rss_collector.rb` の実装
  - RSSフィードの取得
  - RSS/Atomフォーマットのパース
  - Article構造体への変換
- [x] RSSCollectorの統合テスト作成（モックフィード使用）
...
- [x] `lib/tech_news/collectors/github_trending_collector.rb` の実装
  - GitHub Trending APIまたはスクレイピング
  - リポジトリ情報の取得
  - Article形式への変換
- [x] GitHub Collectorのテスト作成
...
- [x] `lib/tech_news/collectors/factory.rb` の実装
...
- [x] `summarize_batch(articles)` メソッドの実装
  - レート制限対策の待機時間
  - 部分的な失敗の処理（継続実行）
  - 進捗ログ
- [x] バッチ処理の統合テスト作成
...
- [x] `lib/tech_news/notifier.rb` の実装
  - Slack Webhook URLの検証
  - `notify(summary)` メソッド
  - Slackメッセージフォーマット構築（Block Kit）
  - HTTPリクエスト送信
  - エラーハンドリングとリトライ
- [x] Notifierのユニットテスト作成（Webhook APIモック）
...
- [x] `notify_batch(summaries)` メソッドの実装
  - 投稿間隔の設定（1秒待機）
  - 部分的な失敗の処理
  - 投稿結果のログ記録
- [x] バッチ投稿の統合テスト作成
...
- [x] `lib/tech_news/orchestrator.rb` の実装
  - 全コンポーネントの初期化
  - `run` メソッド（メインワークフロー）
  - 各フェーズの実行と結果集約
  - 全体的なエラーハンドリング
- [x] Orchestratorの統合テスト作成
...
- [x] `.github/workflows/run-curation.yml` の作成
  - cron設定（日次実行: `0 9 * * *`）
...
- [x] 必須Secretsのドキュメント作成
  - `ANTHROPIC_API_KEY`
  - `SLACK_WEBHOOK_URL`
- [x] Secretsの設定手順をREADMEに追加
- [x] ワークフローでのSecrets利用確認
...
### Task 4.3: ワークフロー最適化
- [x] Gemキャッシュの設定（`actions/cache`）
- [x] タイムアウト設定（10分）
- [x] 失敗通知の設定（GitHub Actions通知）
- [x] 実行時間の計測とログ記録
...
- [x] `config/sources.example.yml` の作成
  - 複数ソースの例
  - コメント付き説明
- [x] `.env.example` の更新（全環境変数）
